 ...
 * select the correct word of a given meaning. It generates options by
 * combining the correct word with three random wrong options from
 * other words in the user's vocabulary.
 */
class MultipleChoiceWordByMeaning extends Component
{
    ...
    public function submitAnswer($selectedOption)
    {
        ...
        if ($isCorrect) {
            $this->word->score = min(100, $this->word->score + config('myapp.multiple_choice_word_by_meaning.score.correct'));
        } else {
            $this->word->score = max(0, $this->word->score + config('myapp.multiple_choice_word_by_meaning.score.incorrect'));
        }
    ...
    {
        return view('livewire.exam.question-types.multiple-choice-word-by-meaning');
    }
}
